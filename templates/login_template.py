...
class LoginTemplate:    
    ...
    def build_ui(self):
        tk.Label(self.root, text="Login to PocketPaws", font=self.font_title, bg=self.bg_app, fg=self.text_main).pack(pady=(40, 10))
        ...
        self.entry_password.pack(fill="x", pady=(0, 25), ipady=5)

        self.create_modern_button(card, "Login 🐾", self.login_callback, "#FFDEB4", "#FFCBA4").pack(fill="x", pady=(0, 15))
        
        tk.Label(card, text="Don't have an account?", font=("Segoe UI", 9), bg=self.bg_card, fg=self.text_main).pack(pady=(5, 5))
        self.create_modern_button(card, "Go to Register", self.register_callback, "#F2F4F4", "#E5E8E8").pack(fill="x")